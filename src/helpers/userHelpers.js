import { getLocalStorage, updateLocalStorage, removeLocalStorage } from './localStorageHelpers';
...
export async function removeConnectedGuestSides(guestSideSessionId) {
  ...
  if (token) {
    try {
      const response = await fetch(API_BASE_URL + '/removeConnectedGuestsSides', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
