      ...
      {( items.length > 0 || (user && user.roleId == 1)) &&
        <div className={styles['item-lister']}>
          <div className={styles['title-container']}>
            ...
            {user && user.roleId == 1 &&
              <>
                <button className={styles['edit-typeItem-button']} onClick={toggleEditTypeItem}>
                  {isEdit ? 'Cancel' : 'Edit'}
                </button>
                {isEdit &&
                  <button className={styles['edit-typeItem-button']} onClick={handleSaveType}>
                    &nbsp;&nbsp;Save&nbsp;&nbsp;
                  </button>
                }
                {isEdit &&
                  <button className={styles['edit-typeItem-button']} onClick={handleRemoveType}>
                    Remove
                  </button>
                }
              </>
          ...
          <div className={styles['item-list']}>
            {user && user.roleId == 1 && isEdit &&
              <>
                <button className={styles['add-item-button']} onClick={toggleAddNewItem}>
