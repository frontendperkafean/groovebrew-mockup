...
const Item = ({ blank, forCart, forInvoice, name: initialName, price: initialPrice, qty: initialQty, imageUrl, id, onPlusClick, onNegativeClick, handleCreateItem, onRemoveClick }) => {
    ...
    <div className={`${styles.item} ${forInvoice ? styles.itemInvoice : ''}`}>
      {!forInvoice && (
        <div className={styles.imageContainer}>
          <img src={previewUrl} alt={itemName} className={styles.itemImage} />
          {blank && (
      ...
      <div className={styles.itemDetails}>
        ...
        {!forInvoice && (
          <div className={styles.itemQty}>
            <svg className={styles.plusNegative} onClick={handleNegativeClick} clipRule="evenodd" fillRule="evenodd" strokeLinejoin="round" strokeMiterlimit="2" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              <path
            ...
            {blank && <input className={styles.itemQtyInput} value={itemQty} onChange={handleQtyChange} disabled={!blank} />}
            <svg className={styles.plusNegative} onClick={handlePlusClick} clipRule="evenodd" fillRule="evenodd" strokeLinejoin="round" strokeMiterlimit="2" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              <path
        ...
        {forInvoice && <p className={styles.itemPriceInvoice}>Rp {itemQty * itemPrice}</p>}
      </div>
      {forCart && (
        <div className={styles.remove} onClick={handleRemoveClick}>ⓧ</div>
      )}
      {blank && <button className={styles.createItem} onClick={handleCreate}>Create Item</button>}
    </div>
  );
};
